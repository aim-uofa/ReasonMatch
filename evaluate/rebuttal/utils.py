...
def scan_dataset_index(testset_root: Path) -> list[dict]:
    entries: list[dict] = []
    meta_paths = sorted(testset_root.rglob("metadata.json"))
    for meta_path in meta_paths:
        ...
        meta_info = meta_data.get("meta", {}) or {}
        assignment = meta_data.get("assignment", {}) or {}

        dataset_key = meta_info.get("dataset_key") or meta_info.get("dataset")
        dataset = meta_info.get("dataset") or dataset_key
        stage = meta_info.get("stage") or assignment.get("stage_id")
        sample_id = meta_info.get("sample_id") or meta_path.parent.name
        label_scheme = meta_info.get("label_scheme")

        relative_parts = meta_path.relative_to(testset_root).parts
        split_dir = relative_parts[1] if len(relative_parts) >= 3 else ""
        dataset_root = relative_parts[0] if len(relative_parts) >= 3 else ""

        if not label_scheme and split_dir.endswith("_alpha"):
            label_scheme = "alpha"
        elif not label_scheme and split_dir.endswith("_numeric"):
            label_scheme = "numeric"

        if not dataset_key and split_dir:
            if split_dir.endswith("_alpha") or split_dir.endswith("_numeric"):
                dataset_key = split_dir.rsplit("_", 1)[0]
        if not dataset_key:
            dataset_key = dataset_root or "unknown"
        if not dataset:
            dataset = dataset_key
        if not stage:
            stage = "unknown"
        ...
        entries.append(
            {
                "metadata": meta_path.relative_to(testset_root).as_posix(),
                "dataset": dataset,
                "dataset_key": dataset_key,
                "stage": stage,
                "sample_id": sample_id,
                "viewA": meta_data.get("image1"),
                "viewB": meta_data.get("image2"),
...
def _resolve_image_path(testset_root: Path, meta_path: Path, image_value: str) -> str:
    if not image_value:
        return image_value
    ...
    if image_path.parts and image_path.parts[0] == meta_path.parent.name:
        stripped = (
            Path(*image_path.parts[1:])
            if len(image_path.parts) > 1
            else Path(image_path.name)
        )
        candidates.append(meta_path.parent / stripped)
    candidates.append(meta_path.parent / image_path)
    candidates.append(testset_root / image_path)
    if "testset/" in image_value:
        tail = image_value.split("testset/")[-1]
        candidates.append(testset_root / tail)
...
def read_metadata(testset_root: Path, relative_path: str) -> dict:
    ...
    meta_data["image1"] = _resolve_image_path(
        testset_root, meta_path, meta_data.get("image1", "")
    )
    meta_data["image2"] = _resolve_image_path(
        testset_root, meta_path, meta_data.get("image2", "")
    )
    return meta_data
...
def compare_mappings(
    gt: dict[str, Any], pred: dict[str, Any] | None
